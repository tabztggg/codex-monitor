...
**最新版本：[v0.4.1](https://github.com/tabztggg/codex-monitor/releases/tag/v0.4.1)** ·
修复并发聊天校准与重复日志统计，改进固定顶栏导航 · [更新日志与升级说明](CHANGELOG.md)
...
以下为 v0.4.0 真实应用界面截图，使用虚构账号、项目、任务和用量，不包含私人数据。
截图展示完整 7 列；项目分组默认关闭，可按需开启。

### 用量一目了然

当前账号额度、跨账号用量估算、任务明细分区展示；明确标注周期开始、结束／重置和数据截至。

![Codex Monitor v0.4.0 中文仪表盘：账号额度、跨账号估算、周期起止和任务明细](assets/screenshots/overview-zh.jpg)
...
校准按聊天分别追踪额度读数，允许并发聊天的快照正常滞后；同一聊天回退、费用未知或观察中断的区间不参与校准。
同刻并发记录合并计算，切换账号窗口保留已见最高值，分段日志使用稳定样本标识避免覆盖或重复。
升级样本格式时继续显示旧参考，按既有归档范围重建兼容样本，达到 5 个百分点后才替换，不扩大归档扫描范围。
v0.4.1 的样本标识与持久样本格式为 v3，解析缓存中的校准数据惰性升级为 v4；无需清空缓存，额度归因账本继续保留。
账号额度占比只分配开始观测后的额度增量；同一任务更早的消耗可能尚未归因，不能直接等同于等效消耗。

顶部导航保持可见，从任务明细、趋势或估算依据可随时返回用量总览；窄屏换行后跳转也会避开顶栏。
...
从 v0.2.0 之前的旧日志解析器升级时，会重建归档统计缓存和额度分配基线。此前已经消耗的
...
页面按“当前账号额度 → 跨账号用量估算 → 任务明细”分区；顶部导航可跳转到总览、任务和趋势。
账号卡片集中显示账号来源、剩余额度与重置时间。跨账号区域使用独立指标卡，统计周期与对比套餐紧邻标题；
归档统计入口位于任务标题旁，任务名称下方显示所属项目。趋势和详细说明位于表格下方。
刷新间隔直接显示在右上角，“更多操作”提供重新统计和刷新状态。所有窗口宽度
默认使用完整表格，宽度不足时左右滚动。选择精简视图后显示任务、所选指标和状态，
展开任务可查看其他指标；只有完整表格提供列显隐设置。视图与精简指标会被记住。
零额度弱化显示，时间进度偏差在五个百分点
以内采用中性提示。
...
范围合计明确标注“跨账号”，仅汇总所选范围内的本地记录。本周期按当前账号的额度
重置窗口划定，分别显示开始、结束／重置和数据截至时间，并标注监控电脑的时区；
“数据截至”是此次统计的截止时刻，不是周期结束时间。
